...
public:
	...
	CMemoryPool(int iBlockNum, bool bLockFlag = false)
	{
		...
		InitializeSRWLock(&m_srwMemPool);
	}
	...
	DATA	*Alloc(bool bPlacementNew = true)
	{
		...
		memset((DATA *)(stpBlock + 1), 0, sizeof(DATA));
	...
	void Lock()
	{
		AcquireSRWLockExclusive(&m_srwMemPool);
	}

	void Unlock()
	{
		ReleaseSRWLockExclusive(&m_srwMemPool);
	}
